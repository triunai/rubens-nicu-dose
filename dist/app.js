...
const MEDICATIONS = {
    cefotaxime: {
        ...
        instructions: {
            indication: "Treatment of susceptible infections and meningitis, guided by organism sensitivities, disease severity and discussion with microbiology.",
            route: "By intramuscular or by slow intravenous injection over 3 to 5 minutes.",
            preparation: "Reconstitute a 500 mg vial with 1.7 mL water for injection to give a 250 mg/mL solution.",
        ...
        externalReferences: [
            {
                ...
                summary: "Official U.S. label reviewed on 2026-04-03. Lists neonatal doses of 50 mg/kg every 12 hours for 0 to 1 week and 50 mg/kg every 8 hours for 1 to 4 weeks, and shows 500 mg vial preparation at approximately 230 mg/mL IM or 50 mg/mL IV."
            },
            {
                ...
                summary: "Neonatal unit administration guide reviewed on 2026-04-03. Severe infection or meningitis lines match the 50 mg/kg severe pathway and warn that different brands have different displacement values, so brand-specific preparation instructions should be checked."
            },
            {
                ...
                summary: "Paediatric monograph last reviewed October 2025. States IV injections should be given over 3 to 5 minutes and warns rapid IV injection has caused life-threatening arrhythmias. For neonates it defers to neonatal protocols."
            }
        ],
        limitations: [
...
const ui = {
    medicine: queryElement("#medicine"),
    ageBand: queryElement("#ageBand"),
    weightKg: queryElement("#weightKg"),
    concentrationMgMl: queryElement("#concentrationMgMl"),
    copyLinkButton: queryElement("#copyLinkButton"),
    resetButton: queryElement("#resetButton"),
    referenceBlock: queryElement("#referenceBlock"),
    documentationBlock: queryElement("#documentationBlock"),
    limitationsBlock: queryElement("#limitationsBlock"),
    errorState: queryElement("#errorState"),
    maxWarning: queryElement("#maxWarning"),
    doseMg: queryElement("#doseMg"),
    doseMgPerKg: queryElement("#doseMgPerKg"),
    drawUpMl: queryElement("#drawUpMl"),
    drawUpMeta: queryElement("#drawUpMeta"),
    frequencyText: queryElement("#frequencyText"),
    dosesPerDayText: queryElement("#dosesPerDayText"),
    dailyTotal: queryElement("#dailyTotal"),
    dailyTotalMeta: queryElement("#dailyTotalMeta"),
    auditLineOne: queryElement("#auditLineOne"),
    auditLineTwo: queryElement("#auditLineTwo"),
    auditLineThree: queryElement("#auditLineThree"),
    severityInputs: document.querySelectorAll("input[name='severity']")
};
...
function selectedSeverityRule(ageBand) {
...
function populateAgeBandSelect() {
...
function renderLimitationsBlock() {
...
function applyUrlState() {
    ...
    state.ageBandId = medication.ageBands.some((band) => band.id === bandId) ? bandId : medication.ageBands[0].id;
...
function clearMaxWarning() {
    ui.maxWarning.hidden = true;
    ui.maxWarning.textContent = "";
}
function showMaxWarning(message) {
...
function calculate() {
    ...
    if (ageBand.maxDailyMg && dailyMaxMg > ageBand.maxDailyMg) {
        showMaxWarning(`The selected schedule reaches ${formatNumber(dailyMaxMg)} mg/day, above the sheet maximum of ${formatNumber(ageBand.maxDailyMg)} mg/day. Independent review is needed before use.`);
    }
}
